 ...
 * INTERNAL:
 * <p>
 * <b>Purpose</b>: Define a fully qualified field name.<p>
...
public class DatabaseField implements Cloneable, Serializable, CoreField {
    ...
    /**
     * Respective Java type desired for the field's value, used to optimize
     * performance and for binding. PERF: Allow direct variable access from
     * getObject.
     */
    ...
    /**
     * Respective JDBC type of the field's value. This overrides the class type,
     * which the JDBC type is normally computed from. PERF: Allow direct
     * variable access from getObject.
     */
    public int sqlType;

    /**
     * Store normal index of field in result set to optimize performance. PERF:
     * Allow direct variable access from getIndicatingNoEntry.
     */
    public int index;
    ...
    /**
     * If this is set, it will be used in determining equality (unless
     * delimiters are used) and the hashcode.
     *
    ...
    /**
     * setting to true will cause getNameForComparisons to lazy initialize
     * nameForComparisons using the value from getName().toUpperCase().
     */
     ...
     * is created. Used by ObjectLevelReadQuery ResultSetAccessOptimization.
     */
     ...
     * value should only apply for string fields we set it to 0 to indicate that
     * it was not specified and rely on the default (255) to come from
    ...
    /**
     * Determine whether the receiver is equal to a DatabaseField. Return true
     * if the receiver and field have the same name and table. Also return true
     * if the table of the receiver or field are unspecified, ie. have no name.
     */
    ...
    /**
     * Returns this fields name with database delimiters if useDelimiters is
     * true. This method should be called any time the field name is requested
     * for writing SQL.
     */
    ...
    /**
     * Return the JDBC type that corresponds to the field. The JDBC type is
     * normally determined from the class type, but this allows it to be
     * overridden for types that do not match directly to a Java type, such as
     * MONEY or ARRAY, STRUCT, XMLTYPE, etc. This can be used for binding or
    ...
    /**
     * Used to specify whether the column should be included in the primary on
     * the database table.
     */
     ...
     * Set the unqualified name of the field.
     *
     * If the name contains database delimiters, they will be stripped and a
     * flag will be set to have them added when the DatabaseField is written to
     * SQL
     */
    ...
    /**
     * Used to specify whether the column should be included in primary key on
     * the database table.
     */
    ...
    /**
     * Set the JDBC type that corresponds to the field. The JDBC type is
     * normally determined from the class type, but this allows it to be
     * overridden for types that do not match directly to a Java type, such as
     * MONEY or ARRAY, STRUCT, XMLTYPE, etc. This can be used for binding or
     * stored procedure usage.
    ...
    /**
     * Set the Java class type that corresponds to the field. The JDBC type is
     * determined from the class type, this is used to optimize performance, and
     * for binding.
     */
    @Override
    ...
    /**
     * INTERNAL: Sets the useUpperCaseForComparisons flag which is used to force
     * using the uppercase version of the field's name to determine field
     * equality and its hashcode, but will still use the original name when
     * writing/printing operations. If this isn't a change, it is ignored,
     * otherwise it sets the nameForComparisons to null.
     */
    ...
    /**
     * INTERNAL: sets the string to be used for equality checking and
     * determining the hashcode of this field. This will overwrite the
     * useUpperCaseForEquality setting with the passed in string.
     */
    ...
    /**
     * INTERNAL: gets the string used for comparisons and in determining the
     * hashcode.
     */
