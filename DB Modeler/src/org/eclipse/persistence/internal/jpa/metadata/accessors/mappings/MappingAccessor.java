...
/**
 * INTERNAL: An abstract mapping accessor. Holds common metadata for all
 * mappings.
 *
 * Key notes: - any metadata mapped from XML to this class must be compared in
 * the equals method. - any metadata mapped from XML to this class must be
 * handled in the merge method. (merging is done at the accessor/mapping level)
 * - any metadata mapped from XML to this class must be initialized in the
 * initXMLObject method. - methods should be preserved in alphabetical order.
 *
...
public abstract class MappingAccessor extends MetadataAccessor {
    ...
    /**
     * INTERNAL: Subclasses that support converts need to override this method
     * otherwise an exception will be thrown from those accessors that do not
     * support them when a user has defined them on that accessor.
     */
    ...
    /**
     * INTERNAL: Add a JPA convert annotation to the converts list. If it is a
     * map key convert, pass it on to the map key converts list.
     */
    ...
    /**
     * INTERNAL: Process an attribute override for either an embedded object
     * mapping, or an element collection mapping containing embeddable objects.
     */
    ...
    /**
     * INTERNAL: For merging and overriding to work properly, all ORMetadata
     * must be able to compare themselves for metadata equality.
     */
    @Override
    ...
    /**
     * INTERNAL: Process the list of association overrides into a map, merging
     * and overriding any association overrides where necessary with descriptor
    ...
    /**
     * INTERNAL: Return the attribute name for this accessor. This is typically
     * the attribute name on the accessible object (i.e., field or property
     * name), however, if access-methods have been specified, use the name
     * attribute that was specified in XML. (e.g. basic name="sin") and not the
     * property name of the get method from the access-methods specification.
     */
    @Override
    ...
    /**
     * INTERNAL: Process the list of attribute overrides into a map, merging and
     * overriding any attribute overrides where necessary with descriptor level
     * attribute overrides.
     */
    ...
    /**
     * INTERNAL: Used for OX mapping. Those accessors that do not require a
     * separate attribute-type specification for VIRTUAL accessors should
     * override this method. For example, one-to-one and many-to-one will its
     * target-entity. variable-one-to-one will use its target-interface.
     */
    ...
    /**
     * INTERNAL: Subclasses should override this method to return the
     * appropriate column for their mapping.
     *
     * @see BasicAccessor
    ...
    /**
     * INTERNAL: Given the potential converts return them for processing unless
     * there are overrides available from the descriptor.
     */
    ...
    /**
     * INTERNAL: Process column metadata details into a database field. This
     * will set correct metadata and log defaulting messages to the user. It
     * also looks for an attribute override.
     *
    ...
    /**
     * INTERNAL: Return the default table to hold the foreign key of a MapKey
     * when and Entity is used as the MapKey
     */
    ...
    /**
     * INTERNAL: Return the foreign key to use with this mapping accessor. This
     * method will look for association overrides and use the foreign key from
     * it instead (do as the association override says).
     */
    ...
    /**
     * INTERNAL: Returns the get method name of a method accessor. Note, this
     * method should not be called when processing field access.
     */
    ...
    /**
     * INTERNAL: Return the join columns to use with this mapping accessor. This
     * method will look for association overrides and use those instead if some
     * are available. This method will validate the join columns and default any
     * where necessary.
     */
    ...
    /**
     * INTERNAL: This method will validate the join columns and default any
     * where necessary.
     */
    ...
    /**
     * INTERNAL: Return the owning descriptors of this accessor. In most cases
     * this is a single descriptor. Multiples can only exist when dealing with
    ...
    /**
     * INTERNAL: Return the map key if this mapping accessor employs one. Those
     * accessors that support it should override this method.
     *
     * @see CollectionAccessor
     * @see ElementCollectionAccessor
     */
    ...
    /**
     * INTERNAL: Return the map key reference class for this accessor if
     * applicable. It will try to extract a reference class from a generic
     * specification. Parameterized generic keys on a MappedSuperclass will
     * return void.class. If no generics are used, then it will return
     * void.class. This avoids NPE's when processing JPA converters that can
     * default (Enumerated and Temporal) based on the reference class.
     */
     ...
     * INTERNAL: Return the map key reference class for this accessor if
     * applicable. It will try to extract a reference class from a generic
     * specification. If no generics are used, then it will return void.class.
     * This avoids NPE's when processing JPA converters that can default
     * (Enumerated and Temporal) based on the reference class.
     *
    ...
    /**
     * INTERNAL: Return the raw class for this accessor. E.g. For an accessor
     * with a type of java.util.Collection<Employee>, this method will return
     * java.util.Collection. To check for the attribute type we must go through
     * the method calls since some accessors define the attribute type through a
     * target entity specification. Do not access the m_attributeType variable
    ...
    /**
     * INTERNAL: Return the raw class with any generic specifications for this
     * accessor. E.g. For an accessor with a type of
     * java.util.Collection<Employee>, this method will return
     * java.util.CollectionEmployee. To check for the attribute type we must go
     * through the method calls since some accessors define the attribute type
     * through a target entity specification. Do not access the m_attributeType
    ...
    /**
     * INTERNAL: Return the mapping accessors associated with the reference
     * descriptor.
     */
    ...
    /**
     * INTERNAL: Return the reference class for this accessor. By default the
     * reference class is the raw class. Some accessors may need to override
     * this method to drill down further. That is, try to extract a reference
     * class from generics.
     */
    ...
    /**
     * INTERNAL: Attempts to return a reference class from a generic
     * specification. Note, this method may return null.
     */
    ...
    /**
     * INTERNAL: Return the reference descriptors table. By default it is the
     * primary key table off the reference descriptor. Subclasses that care to
     * return a different class should override this method.
     *
    ...
    /**
     * INTERNAL: Returns the set method name of a method accessor. Note, this
     * method should not be called when processing field access.
     */
    ...
    /**
     * INTERNAL: Those accessors that do not require a separate attribute-type
    ...
    /**
     * INTERNAL: Method should be overridden by those accessors that accept and
     * use a map key.
     *
     * @see CollectionAccessor
     * @see ElementCollectionAccessor
    ...
    /**
     * INTERNAL: Indicates whether the specified annotation is present on the
     * annotated element for this accessor. Method checks against the metadata
     * complete flag.
     */
    @Override
    ...
    /**
     * INTERNAL: Return true if this accessor represents a basic collection
     * mapping.
     */
    ...
    /**
     * INTERNAL: Return true if this accessor represents a direct collection
     * mapping, which include basic collection, basic map and element collection
     * accessors.
     */
    ...
    /**
     * INTERNAL: Return true if this accessor represents an element collection
     * that contains embeddable objects.
     */
    ...
    /**
     * INTERNAL: Return true if this accessor represents an aggregate id
     * mapping.
     */
    ...
    /**
     * INTERNAL: Return true if this represents an enum type mapping. Will
     * return true if the accessor's reference class is an enum or if enumerated
     * metadata exists.
     */
    ...
    /**
     * INTERNAL: Return true if this accessor is a mapped key map accessor. It
     * is a map key accessor for two reasons, it's a map and it does not have a
     * map key specified. NOTE: we can't check for a map key class since one may
     * not have been explicitly specified. In this case, a generic value must be
     * set and we check for one when adding accessors (and in turn set the map
     * key class at that point)
     */
    ...
    /**
     * INTERNAL: Return true if this accessor has been processed. If there is a
     * mapping set, we have processed this accessor.
     */
    @Override
    ...
    /**
     * INTERNAL: Returns true if the given class is valid for
    ...
    /**
     * INTERNAL: Return true if this accessor represents a variable one to one
     * mapping.
     */
    ...
    /**
     * INTERNAL: Process an association override for either an embedded object
     * mapping, or a map mapping (element-collection, 1-M and M-M) containing an
     * embeddable object as the value or key. This method should be implemented
     * in those accessors that support association overrides. An exception is
     * thrown otherwise the association is called against an unsupported
    ...
    /**
     * INTERNAL: Process the association overrides for the given embeddable
     * mapping which is either an embedded or element collection mapping.
     * Association overrides are used to specify different keys to a shared
     * mapping.
     */
     ...
     * Attribute overrides are used to apply the correct field name translations
     * of direct fields. Note an embedded object mapping may be supported as the
     * map key to an element-collection, 1-M and M-M mapping.
     */
    ...
    /**
     * INTERNAL: Process the map metadata if this is a valid map accessor. Will
     * return the map key method name that should be use, null otherwise.
     *
     * @see CollectionAccessor
     * @see ElementCollectionAccessor
     */
    ...
    /**
     * INTERNAL: Process a Convert annotation or convert element to apply to
     * specified EclipseLink converter (Converter, TypeConverter,
     * ObjectTypeConverter) to the given mapping.
     */
    ...
    /**
     * INTERNAL: Process an Enumerated setting. The method may still be called
     * if no Enumerated metadata has been specified but the accessor's reference
    ...
    /**
     * INTERNAL: Process a lob specification. The lob must be specified to
     * process and create a lob type mapping.
     */
    ...
    /**
     * INTERNAL: Process a convert value which specifies the name of an
     * EclipseLink converter to process with this accessor's mapping.
     */
    protected void processMappingConverter(DatabaseMapping mapping, String convertValue, List<ConvertMetadata> converts, MetadataClass referenceClass, MetadataClass referenceClassWithGenerics, boolean isForMapKey) {
        ...
        } else // Check for original JPA converters. Check for an enum first since
    ...
    /**
     * INTERNAL: Process a mapping key converter either from an EclipseLink
     * convert specification or a JPA converter specification (map-key-convert,
    ...
    /**
     * INTERNAL: Process the join columns for the owning side of a one to one
     * mapping. The default pk and fk field names are used only with single
    ...
    /**
     * INTERNAL: Adds properties to the mapping. They can only come from one
     * place, therefore if we add the same one twice we know to throw an
     * exception.
     */
    ...
    /**
     * INTERNAL: Subclasses should call this method if they want the warning
     * message or override the method if they want/support different behavior.
     *
     * @see BasicAccessor
     */
    ...
    /**
     * INTERNAL: Process a potential serializable attribute. If the class
     * implements the Serializable interface then set a
     * SerializedObjectConverter on the mapping.
     */
     ...
     * Method assume that the reference class has been set on the mapping before
     * calling this method.
     */
    protected void setIndirectionPolicy(ContainerMapping mapping, String mapKey, boolean usesIndirection) {
        ...
        if (!containerPolicySet) {
            ...
            } else // Use the supplied collection class type if its not an interface
            {
    ...
    /**
     * INTERNAL: This will do three things: 1 - process any common level
     * metadata for all mappings. 2 - add the mapping to the internal
     * descriptor. 3 - store the actual database mapping associated with this
     * accessor.
     *
     * Calling this method is a must for all mapping accessors since it will
     * help to: 1 - determine if the accessor has been processed, and 2 - sub
     * processing will may need access to the mapping to set its metadata.
     */
    ...
    /**
     * INTERNAL: An override mapping is created when an association override is
     * specified to a relationship accessor on an embeddable class. For any
     * non-owning relationship accessor referring to this accessor will need its
     * override mapping and not the original mapping from the embeddable so that
     * it can populate the right metadata.
     */
    ...
    /**
     * INTERNAL: Update the primary key field on the owning descriptor the
     * override field given.
     */
    ...
    /**
     * INTERNAL: Returns true if this mapping or class uses property access. In
     * an inheritance hierarchy, the subclasses inherit their access type from
    ...
    /**
     * INTERNAL: Returns true if this mapping or class uses virtual access. In
     * an inheritance hierarchy, the subclasses inherit their access type from
     * the parent (unless there is an explicit access setting).
     */
    ...
    /**
     * INTERNAL: Returns true if this mapping or class uses property field. In
     * an inheritance hierarchy, the subclasses inherit their access type from
     * the parent (unless there is an explicit access setting).
     */
