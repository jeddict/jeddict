...
/**
 * An embedded relationship accessor. It may define all the same attributes as
 * an entity, therefore, it also must handle nesting embedded's to the nth
 * level. An embedded owning descriptor is a reference back to the actual owning
 * entity's descriptor where the first embedded was discovered.
 *
...
public class EmbeddedAccessor extends MappingAccessor {
    ...
    /**
     * INTERNAL: Subclasses that support key converts need to override this
     * method.
     */
    @Override
    ...
    /**
     * INTERNAL: Called when process the mapsId metadata. The id fields for this
     * owning descriptor must have it's id fields update to those from the one
     * to one accessor that maps them. We process embedded and embedded id
     * mappings first, so by default they get mapped and processed as they
     * normally would. When we go through the relationship accessors and
     * discover a mapsId we then need to make some updates to our list of
     * primary key fields.
     */
