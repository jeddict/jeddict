...
/**
 * <p>
 * <b>Purpose</b>:Two objects can be considered to be related by aggregation if
 * there is a strict 1:1 relationship between the objects. This means that the
 * target (child or owned) object cannot exist without the source (parent)
 * object.
 *
 * In TopLink, it also means the data for the owned object is stored in the same
 * table as the parent.
 *
 * @author Sati
...
public class AggregateObjectMapping extends AggregateMapping implements RelationalMapping, MapKeyMapping, EmbeddableMapping {
    ...
    /**
     * If <em>all</em> the fields in the database row for the aggregate object
     * are NULL, then, by default, the mapping will place a null in the
     * appropriate source object (as opposed to an aggregate object filled with
     * nulls). To change this behavior, set the value of this variable to false.
     * Then the mapping will build a new instance of the aggregate object that
     * is filled with nulls and place it in the source object.
     */
    ...
    /**
     * Map the name of a field in the aggregate descriptor to a field in the
     * source table.
     */
    /**
     * 322233 - changed to store the source DatabaseField to hold Case and other
     * colunm info
     */
    protected Map<String, DatabaseField> aggregateToSourceFields;

    /**
     * Map of nested attributes that need to apply an override name to their a
     * nested aggregate mapping's database field. Aggregate to source fields map
     * is the existing EclipseLink functionality and works well when all
    ...
    /**
     * List of converters to apply at initialize time to their cloned aggregate
     * mappings.
     */
     ...
     * Called when the selection query is being initialized to add the fields
     * for the map key to the query
     */
    @Override
    ...
    /**
     * INTERNAL: Used when initializing queries for mappings that use a Map
     * Called when the insert query is being initialized to ensure the fields
     * for the map key are in the insert query
     */
    @Override
    ...
    /**
     * PUBLIC: Add a field name translation that maps from a field name in the
    ...
    /**
     * PUBLIC: Add a field translation that maps from a field in the source
     * table to a field name in the aggregate descriptor.
     */
    ...
    /**
     * INTERNAL: In JPA users may specify a maps id mapping on a shared
     * embeddable descriptor. These mappings need to be set to read-only at
    ...
    /**
     * INTERNAL: Add a nested field translation that maps from a field in the
     * source table to a field name in a nested aggregate descriptor. These are
     * handled slightly different that regular field translations in that they
     * are unique based on the attribute name. It solves the case where multiple
    ...
    /**
     * INTERNAL: In JPA users may specify overrides to apply to a many to many
     * mapping on a shared embeddable descriptor. These settings are applied at
     * initialize time, after the reference descriptor is cloned.
     */
    @Override
    ...
    /**
     * INTERNAL: In JPA users may specify overrides to apply to a unidirectional
     * one to many mapping on a shared embeddable descriptor. These settings are
    ...
    /**
     * INTERNAL: For mappings used as MapKeys in MappedKeyContainerPolicy. Add
     * the target of this mapping to the deleted objects list if necessary
     *
     * This method is used for removal of private owned relationships.
     * AggregateObjectMappings are dealt with in their parent delete, so this is
     * a no-op.
     *
     * @param object
    ...
    /**
     * INTERNAL: Return whether all the aggregate fields in the specified row
     * are NULL.
     */
    ...
    /**
     * PUBLIC: If <em>all</em> the fields in the database row for the aggregate
     * object are NULL, then, by default, the mapping will place a null in the
     * appropriate source object (as opposed to an aggregate object filled with
     * nulls). This behavior can be explicitly set by calling #allowNull(). To
     * change this behavior, call #dontAllowNull(). Then the mapping will build
     * a new instance of the aggregate object that is filled with nulls and
     * place it in the source object. In either situation, when writing, the
     * mapping will place a NULL in all the fields in the database row for the
     * aggregate object.
     *
     * Note: Any aggregate that has a relationship mapping automatically does
     * not allow null.
     */
    ...
    /**
     * INTERNAL: Return whether the query's backup object has an attribute value
     * of null.
     */
    ...
    /**
     * INTERNAL: Clone and prepare the selection query as a nested batch read
     * query. This is used for nested batch reading.
     */
    ...
    /**
     * INTERNAL: Build and return an aggregate object from the specified row. If
     * a null value is allowed and all the appropriate fields in the row are
     * NULL, return a null. If an aggregate is referenced by the target object,
     * return it (maintain identity) Otherwise, simply create a new aggregate
    ...
    /**
     * INTERNAL: Wrap the aggregate represented by this mapping in a cachekey so
     * it can be processed my methods down the stack.
     *
     * @param owningCacheKey - the cache key holding the object to extract the
     * aggregate from
     * @return
     */
    ...
    /**
     * INTERNAL: Used to allow object level comparisons. In the case of an
     * Aggregate which has no primary key must do an attribute by attribute
     * comparison.
     */
    @Override
    ...
    protected void writeToRowFromAggregate(AbstractRecord record, Object object, Object attributeValue, AbstractSession session, WriteType writeType) throws DescriptorException {
        ...
        } else if (!session.isClassReadOnly(attributeValue.getClass())) {
            getObjectBuilder(attributeValue, session).buildRow(record, attributeValue, session, writeType);
        }
    }

    /**
     * INTERNAL: Write the aggregate values into the parent row for shallow
     * insert.
     */
    ...
    /**
     * INTERNAL: Write the aggregate values into the parent row for update after
     * shallow insert.
     */
    protected void writeToRowFromAggregateForUpdateAfterShallowInsert(AbstractRecord record, Object object, Object attributeValue, AbstractSession session, DatabaseTable table) throws DescriptorException {
        ...
        } else if (!session.isClassReadOnly(attributeValue.getClass()) && !isPrimaryKeyMapping()) {
            getObjectBuilder(attributeValue, session).buildRowForUpdateAfterShallowInsert(record, attributeValue, session, table);
        }
    }

    /**
     * INTERNAL: Write the aggregate values into the parent row for update
    ...
    /**
     * INTERNAL: Build and return a database row built with the values from the
    ...
    protected void writeToRowFromAggregateWithChangeRecord(AbstractRecord record, ChangeRecord changeRecord, ObjectChangeSet objectChangeSet, AbstractSession session, WriteType writeType) throws DescriptorException {
        ...
        } else if (!session.isClassReadOnly(objectChangeSet.getClassType(session))) {
    ...
    /**
     * INTERNAL: Build and return a database row built with the changed values
     * from the specified attribute value.
     */
    protected void writeToRowFromAggregateForUpdate(AbstractRecord record, WriteObjectQuery query, Object attributeValue) throws DescriptorException {
        ...
        } else if (!query.getSession().isClassReadOnly(attributeValue.getClass())) {
    ...
    /**
     * INTERNAL: Clone the attribute from the original and assign it to the
     * clone.
     */
    @Override
     ...
     * working copy clone. In order to bypass the shared cache when in
     * transaction a UnitOfWork must be able to populate working copies directly
     * from the row.
     */
    @Override
    ...
    /**
     * INTERNAL: Builds a shallow original object. Only direct attributes and
     * primary keys are populated. In this way the minimum original required for
     * instantiating a working copy clone can be built without placing it in the
     * shared cache (no concern over cycles).
     */
    @Override
     ...
     * Return the appropriate type of selectionQuery
     *
     * @return
     */
    @Override
    ...
    /**
     * INTERNAL: Build and return a "template" database row with all the fields
     * set to null.
     */
    ...
    /**
     * INTERNAL: Cascade discover and persist new objects during commit to the
     * map key
     */
    @Override
    ...
    /**
     * INTERNAL: Cascade perform delete through mappings that require the
     * cascade
     */
    @Override
    ...
    /**
     * INTERNAL: Cascade perform removal of orphaned private owned objects from
     * the UnitOfWorkChangeSet
     */
    @Override
    ...
    /**
     * INTERNAL: Cascade registerNew for Create through mappings that require
     * the cascade
     */
    @Override
     ...
     * AggregateCollectionMapping needs each nested embedded mapping to have its
     * own list of aggregate to source field names so that it can apply nested
     * override names to shared aggregate object mappings.
     */
    @Override
    public Object clone() {
    ...
    /**
     * INTERNAL: This method is used to store the FK fields that can be cached
     * that correspond to noncacheable mappings the FK field values will be used
     * to re-issue the query when cloning the shared cache entity
     */
    @Override
    ...
    /**
     * INTERNAL: Convert all the class-name-based settings in this mapping to
    ...
    /**
     * INTERNAL Called when a DatabaseMapping is used to map the key in a
     * collection. Returns the key.
     */
    @Override
    ...
    /**
     * INTERNAL: Create an instance of the Key object from the key information
     * extracted from the map. This may return the value directly in case of a
     * simple key or will be used as the FK to load a related entity.
     */
    @Override
    ...
    /**
     * INTERNAL: Create an instance of the Key object from the key information
     * extracted from the map. This key object may be a shallow stub of the
     * actual object if the key is an Entity type.
     */
    @Override
    ...
    /**
     * INTERNAL Called when a DatabaseMapping is used to map the key in a
     * collection and a join query is executed. Returns the key.
     */
    @Override
     ...
     * Delete the passed object if necessary.
     *
     * This method is used for removal of private owned relationships.
     * AggregateObjectMappings are dealt with in their parent delete, so this is
     * a no-op.
    ...
    /**
     * INTERNAL: This method is called to update collection tables prior to
     * commit.
     */
    @Override
    ...
    /**
     * INTERNAL: Extract the fields for the Map key from the object to use in a
     * query.
     */
    @Override
    ...
    /**
     * INTERNAL: Return any tables that will be required when this mapping is
     * used as part of a join query
     *
     * @return
     */
    @Override
    ...
    /**
     * INTERNAL: Return the selection criteria necessary to select the target
     * object when this mapping is a map key.
     *
     * AggregateObjectMappings do not need any additional selection criteria
     * when they are map keys
     *
     * @return
     */
    @Override
    ...
    /**
     * INTERNAL: Return a collection of the aggregate to source field
     * associations.
     */
    ...
    /**
     * INTERNAL: Return the hashtable that stores aggregate field name to source
     * fields.
     */
    ...
    /**
     * INTERNAL: Return the classification for the field contained in the
     * mapping. This is used to convert the row value to a consistent Java
     * value.
     */
    @Override
    ...
    /**
     * INTERNAL: Return the fields that make up the identity of the mapped
     * object. For mappings with a primary key, it will be the set of fields in
     * the primary key. For mappings without a primary key it will likely be all
     * the fields
     *
     * @return
     */
    ...
    /**
     * INTERNAL: This is used to preserve object identity during a
     * refreshObject() query. Return the object corresponding to the specified
     * database row. The default is to simply return the attribute value.
     */
     ...
     * Return the object corresponding to the specified attribute value. The
     * default is to simply return the backup attribute value.
     */
    ...
    /**
     * INTERNAL: Return the query that is used when this mapping is part of a
     * joined relationship
     *
    ...
    /**
     * INTERNAL: Since aggregate object mappings clone their descriptors, for
     * inheritance the correct child clone must be found.
     */
    @Override
    ...
    /**
     * INTERNAL: If required, get the targetVersion of the source object from
     * the merge manager.
     *
     * Used with MapKeyContainerPolicy to abstract getting the target version of
     * a source key
     *
     * @return
     */
    ...
    /**
     * INTERNAL: Return if the mapping has any ownership or other dependency
     * over its target object(s).
     */
    @Override
     ...
     * The cloned descriptor is then assigned primary keys and table names
     * before initialize. Once the cloned descriptor is initialized it is
     * assigned as reference descriptor in the aggregate mapping. This is a very
     * specific behavior for aggregate mappings. The original descriptor is used
     * only for creating clones and after that the aggregate mapping never uses
     * it. Some initialization is done in postInitialize to ensure the target
     * descriptor's references are initialized.
     */
    @Override
...
//   this.getFields()
    ...
    /**
     * INTERNAL: For an aggregate mapping the reference descriptor is cloned. If
     * the reference descriptor is involved in an inheritance tree, all the
     * parent and child descriptors are cloned also. The cloned descriptors are
     * then assigned primary keys and table names before initialize. This is a
     * very specific behavior for aggregate mappings.
     */
    ...
    /**
     * INTERNAL: Initialize the cloned reference descriptor with table names and
     * primary keys
     */
    ...
    /**
     * INTERNAL: Called when iterating through descriptors to handle iteration
     * on this mapping when it is used as a MapKey
     *
     * @param iterator
     * @param element
     */
    ...
    /**
     * INTERNAL: Return whether this mapping should be traversed when we are
     * locking
     *
     * @return
     */
    ...
    /**
     * INTERNAL: Flags that either this mapping or nested mapping is a JPA id
     * mapping.
     */
    ...
    /**
     * PUBLIC: Return if all the fields in the database row for the aggregate
     * object are NULL, then, by default, the mapping will place a null in the
     * appropriate source object (as opposed to an aggregate object filled with
     * nulls). To change this behavior, set the value of this variable to false.
     * Then the mapping will build a new instance of the aggregate object that
    ...
    /**
     * INTERNAL: Making any mapping changes necessary to use a the mapping as a
     * map key prior to initializing the mapping
     */
    @Override
    ...
    /**
     * INTERNAL: Making any mapping changes necessary to use a the mapping as a
     * map key after initializing the mapping.
     */
    @Override
    ...
    /**
     * INTERNAL: Build an aggregate object from the specified return row and put
     * it in the specified target object. Return row is merged into object after
     * execution of insert or update call according to ReturningPolicy. If not
     * null changeSet must correspond to targetObject. changeSet is updated with
     * all of the field values in the row.
     */
    ...
    /**
     * INTERNAL: Build an aggregate object from the specified row and put it in
    ...
    /**
     * INTERNAL: Rehash any hashtables based on fields. This is used to clone
     * descriptors for aggregates, which hammer field names.
     */
    @Override
    ...
    /**
     * INTERNAL: Return whether this mapping requires extra queries to update
     * the rows if it is used as a key in a map. This will typically be true if
     * there are any parts to this mapping that are not read-only.
     */
    @Override
    ...
    /**
     * INTERNAL: Set a collection of the aggregate to source field name
     * associations.
     */
    ...
    /**
     * INTERNAL: Set the hashtable that stores target field name to the source
     * field name.
     */
    ...
    /**
     * PUBLIC: Configure if all the fields in the database row for the aggregate
     * object are NULL, then, by default, the mapping will place a null in the
     * appropriate source object (as opposed to an aggregate object filled with
     * nulls). To change this behavior, set the value of this variable to false.
     * Then the mapping will build a new instance of the aggregate object that
    ...
    /**
     * INTERNAL: If this mapping is used as the key of a
     * CollectionTableMapMapping, the table used by this mapping will be the
     * relation table. Set this table.
     */
    ...
    /**
     * INTERNAL: Apply the field translation from the sourceField to the
     * mappingField.
     */
    ...
    /**
     * INTERNAL: If field names are different in the source and aggregate
     * objects then the translation is done here. The aggregate field name is
     * converted to source field name from the field name mappings stored.
     */
    ...
    /**
     * INTERNAL: A subclass should implement this method if it wants different
     * behavior. Write the foreign key values from the attribute to the row.
     */
    @Override
    ...
    /**
     * INTERNAL: Get the attribute value from the object and add the appropriate
    ...
    /**
     * INTERNAL: This row is built for shallow insert which happens in case of
     * bidirectional inserts.
     */
    @Override
    ...
    /**
     * INTERNAL: This row is built for update after shallow insert which happens
     * in case of bidirectional inserts. It contains the foreign keys with non
     * null values that were set to null for shallow insert.
     */
    @Override
    ...
    /**
     * INTERNAL: This row is built for update before shallow delete which
     * happens in case of bidirectional inserts. It contains the same fields as
     * the row built by writeFromObjectIntoRowForUpdateAfterShallowInsert, but
    ...
    /**
     * INTERNAL: Get the attribute value from the object and add the changed
     * values to the specified database row.
     */
    @Override
    ...
    /**
     * INTERNAL: Write fields needed for insert into the template for with null
     * values.
     */
    @Override
    ...
    /**
     * INTERNAL: Add a primary key join column (secondary field). If this
     * contain primary keys and the descriptor(or its subclass) has multiple
     * tables (secondary tables or joined inheritance strategy), this should
     * also know the primary key join columns to handle some cases properly.
     */
